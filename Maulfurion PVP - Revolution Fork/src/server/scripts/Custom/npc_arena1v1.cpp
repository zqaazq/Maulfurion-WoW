...
public:
	...
	bool CreateArenateam(Player* player, Creature* me)
	{
		...
		ChatHandler(player->GetSession()).SendSysMessage("Time de arena criado com sucesso.");
	...
	bool OnGossipHello(Player* player, Creature* me)
	{
		if(player->GetArenaTeamId(ArenaTeam::GetSlotByType(ARENA_TEAM_1v1)) == NULL)
			player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, "Criar time 1v1", GOSSIP_SENDER_MAIN, 1, "Tem Certeza?", ARENA_1V1_COST, false);
		else
		{
			if(player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_1v1))
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Sair da fila", GOSSIP_SENDER_MAIN, 3);
			else
			{
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Entrar na fila", GOSSIP_SENDER_MAIN, 2);
				player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, "Deletar Time", GOSSIP_SENDER_MAIN, 5, "Tem Certeza?", 0, false);
			}

			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Mostrar Estatisticas", GOSSIP_SENDER_MAIN, 4);
		}
	...
	bool OnGossipSelect(Player* player, Creature* me, uint32 /*uiSender*/, uint32 uiAction)
	{
		...
		case 1: // Create new Arenateam
			{
				...
				{
					ChatHandler(player->GetSession()).PSendSysMessage("Voce precisa do level %u+ para criar um time 1v1.", ARENA_1V1_MIN_LEVEL);
		...
		case 5: // Disband arenateam
			{
				...
				ChatHandler(player->GetSession()).SendSysMessage("Time de Arena deletado.");
				player->CLOSE_GOSSIP_MENU();
				return true;
			}
			break;
		...
		case 8: // Script Info
			{
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Clique em Criar Time de Arena", GOSSIP_SENDER_MAIN, uiAction);
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Marque a Arena 1v1 e espere", GOSSIP_SENDER_MAIN, uiAction);
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Apos isso APROVEITE !", GOSSIP_SENDER_MAIN, uiAction);
