...
public:
	...
	bool PlayerAlreadyHasThreeProfessions(const Player *pPlayer) const
	{
		uint32 skillCount = 0;
	...
	{
		if (PlayerAlreadyHasThreeProfessions(pPlayer) && !IsSecondarySkill(skill))
			pCreature->TextEmote("You already know three professions!", pPlayer);
		else
		{
